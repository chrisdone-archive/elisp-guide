...
使用Emacs Lisp[[https://zh.wikipedia.org/wiki/Emacs_Lisp][Emacs Lisp_ZH]] （阅读Wiki百科以了解更多技术细节）
编程和使用Python、Scheme、Common Lisp、JavaScript、Ruby等其他语言编程类似。
...
「静态作用域是在Eamcs 24.1加入的，所以说“最近”。而翻译本文时为2020-Aug
  ——译者注」
